...
#定义相关变量
random_1 =22# 样本集选取随机种子
rate_1 = 0.25# 样本集验证集
#读入数据
data =pd.read_excel('社会环境效益数据.xlsx')
...
rf=RandomForestRegressor(n_estimators=22,max_depth=None,min_samples_leaf=2,min_samples_split=3,
...
feat_importance_x = list(feature_importance[:6])
feat_importance_x1 = feat_importance_x + [feature_importance[6]+feature_importance[7],feature_importance[8]+feature_importance[9]
                                          +feature_importance[10]+feature_importance[11]+feature_importance[12]
                                          +feature_importance[13],
                                          feature_importance[14]+feature_importance[15]+feature_importance[16]+
                                          feature_importance[17],feature_importance[18]]
...
feat_importance.nlargest(9,columns='rate').plot(kind='barh')
